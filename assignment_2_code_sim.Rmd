...
f_w_05 <- calculate_delta(S_0, sigma, 1, 0.5, setseed = T)

f_w_0 <- calculate_delta(S_0, sigma, 1, 1e-12, setseed = T)
...
plot(y = f_w_05$D, 
     ...
     xlab=("Stock price"), ylab=("Call Delta"), main = ("Estimated Call Deltas in the Bachelier Model"))
text(4, 9, "True Delta")+ 
...
f_w_05 <- calculate_delta(S_0, sigma, 1, 0.5, BS = TRUE, setseed = T)

f_w_0 <- calculate_delta(S_0, sigma, 1, 1e-12, BS = TRUE, setseed = T)
...
plot(y = f_w_05$D, 
     ...
     main = ("Estimated Call Deltas in the Black Scholes Model"))
text(4, 9, "True Delta")+ 
  lines(S_0_to_plot[remove_from_plot],
       true_derr[remove_from_plot], col = "black")+ 
  lines(y = f_w_05$delta[remove_from_plot], 
...
plot(y = true_delta_err_mean, simulations, type = 'l', lty = 0, 
     xlim = c(0, 6000), ylim = c(0, 1), col = "red",
     ylab = "Standard deviation of relative hedge error", xlab = "#simulations",
     main = ("Polynomial call hedging")) +
  lines(y = pricing_delta_err_3_mean, x = simulations, type = 'b', col = "orange")+
  lines(y = pricing_delta_err_4_mean, x = simulations, type = 'b', col = "orange")+
...
plot(y = lrm_3_mean, simulations, type = 'b', ylim = c(0.0, 1), xlim = c(0, 6500), col = "orange",
     ylab = "Standard deviation of relative hedge error", xlab = "#simulations",
     main = ("Polynomial call hedging - LRM and Pathwise")) +
...
```{r}
if(params$save_plots == TRUE){
  png("s3densplot.png", width = 800, height = 600)
}
bw <- c(0.02837, 0.02281, 0.01074, 0.794)
par(mfrow=c(2, 2))
count <- 0
for(time_to_mat in c(1, 0.5, 0.1, 0.01)){
  count <- count + 1
  delta_sim <- calculate_delta(S_0, sigma, time_to_mat, 0.5, BS = TRUE, setseed = T, LRM = T)$D
  density(delta_sim) %>% 
    plot(main = paste0("Density for deltas with T = ", time_to_mat), 
         xlab = paste0("sd = ", round(sd(delta_sim), 5), 
                       ", mean = ", round(mean(delta_sim), 5), 
                       ", kernel = gaussian",
                       ", bw = ", bw[count]))
}
if(params$save_plots == TRUE){
...
f_w_05 <- calculate_delta(S_0, sigma, 1, 0.5, BS = TRUE, digital = TRUE, LRM = TRUE, setseed = TRUE)

f_w_0 <- calculate_delta(S_0, sigma, 1, 1e-12, BS = TRUE, digital = TRUE, LRM = TRUE, setseed = TRUE)
...
plot(y = f_w_05$D, 
     ...
     main = ("Estimated Digital Call Deltas in the Black Scholes Model"))
text(4, 9, "True Delta")+ 
  lines(S_0_to_plot[remove_from_plot],
       true_derr[remove_from_plot], col = "black")+ 
  lines(y = f_w_05$delta[remove_from_plot], 
...
plot(y = delta_lrm_3_mean, simulations, type = 'b', ylim = c(0.0, 1), xlim = c(0, 6500), col = "blue",
     ylab = "Standard deviation of relative hedge error", xlab = "#simulations",
     main = ("Polynomial Digital Call hedging")) +
...
text(6100, delta_lrm_5_mean[8], paste0("5"), col = "blue", adj = 0)
text(6100, delta_lrm_7_mean[8], paste0("7"), col = "blue", adj = 0)
text(6200, std_5_mean[8], paste0("5"), col = "orange", adj = 0)
text(6200, std_7_mean[8], paste0("7"), col = "orange", adj = 0)
text(0, true_delta_err_mean[1]+0.02, paste0("Heding with true Delta"), col = "red", adj = 0)
...
plot(delta_var[-1], x = eps_seq[-1], xlim = c(.22, 10), type = 'l', col = "red",
     ylab = "Variance of D", xlab = "epsilon") +
...
f_w_05 <- calculate_delta(S_0, sigma, 1, 0.5, BS = TRUE, digital = TRUE, 
                          LRM = TRUE, MM = TRUE, eps = eps, setseed = TRUE)

f_w_0 <- calculate_delta(S_0, sigma, 1, 1e-12, BS = TRUE, digital = TRUE, 
                         LRM = TRUE, MM = TRUE, eps = eps, setseed = TRUE)
...
plot(y = f_w_05$D, 
        x = S_0_to_plot, col = "grey", type = "p",
     #xlim = c(0.2, 1.7), ylim = c(-0.1, 1.3), 
     xlab=("Stock price"), ylab=("Call Delta"), ylim = c(-2, 15), xlim = c(0.1, 2.1),
     main = ("Estimated Call Deltas in the Black Scholes Model using the Mixed Method"))
text(4, 9, "True Delta")+ 
  lines(S_0_to_plot[remove_from_plot],
       true_derr[remove_from_plot], col = "black")+ 
  lines(y = f_w_05$delta[remove_from_plot], 
...
text(6200, std_3_mean[8], paste0("3"), col = "orange", adj = 0)
text(6200, std_8_mean[8], paste0("8"), col = "orange", adj = 0)
text(6100, lrm_5_mean[8], paste0("5"), col = "blue", adj = 0)
text(6100, lrm_7_mean[8], paste0("7"), col = "blue", adj = 0)
text(6200, std_5_mean[8], paste0("5"), col = "orange", adj = 0)
text(6200, std_7_mean[8], paste0("7"), col = "orange", adj = 0)
text(0, true_delta_err_mean[1]+0.02, paste0("Heding with true Delta"), col = "red", adj = 0)
