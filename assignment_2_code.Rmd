...
#   pull()
# 
# zeroes <- c(rep(0, length(S_0))) %>% 
#   as_tibble() %>% 
#   rename(zeroes = value)
# ones <- c(rep(1, length(S_0))) %>% 
#   as_tibble() %>% 
#   rename(ones = value)
# X <- ones %>%
#   bind_cols(poly(S_0, 7, raw=TRUE) %>%
#   as_tibble()) %>%
#   as.matrix()
# 
# poly_6 <- poly(S_0, 6, raw=TRUE)
# Y <- zeroes %>% 
#   bind_cols(ones) %>% 
#   bind_cols(poly_6) %>% 
#   as.matrix()
...
# }
# 
# crit <- function(w, C, D, coefs){
#   C_est <- c_xs %*% coefs
#   D_est <- d_xs %*% coefs[-1]
#   first_part <- (C - C_est)^2
#   second_part <- (D - D_est)^2
#   return(w * sum(first_part) + (1-w) * sum(second_part))
# }
# 
# C <- S_T
# theta <- function(w) {
#   return(inv(w * t(X) %*% X + (1 - w) * t(Y) %*% Y) %*% 
#   (w * t(X) %*% C + (1 - w) * t(Y) %*% D))
# }
...
for(j in 1:10){
  for(i in 1:length(simulations)){
    ...
    St <- S_0
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, true_delta, list(K = K, sigma = sigma))
...
# err_own <- sqrt(sum((Vt - sapply(St, FUN = function(x)max(x - K, 0)) - mean(Vt - sapply(St, FUN = function(x)max(x - K, 0))))^2)/num_rep)
#plot(err)
```
...
plot(y = true_delta_err, simulations, type = 'l', ylim = c(0.01, 0.08), col = "red") +
  lines(y = w_05_delta_err, x = simulations, type = 'l', col = "blue")+
  lines(y = pricing_delta_err, x = simulations, type = 'l', col = "orange")
```

```{r}
St <- c(rep(0, 1000)); num_rep <- 1000
err_vec <- St;Vt <- 0
for(i in 1:num_rep){
    at <- 0; bt <- 0;Vt <- 0
    S_0 <- c(rep(1, length(1)))
    St <- S_0
  for(t in seq(dt, T, dt)){
    St <- St + sigma * sqrt(dt) * rnorm(1)
    Vt <- at * St + bt
    at <- pnorm((St-K)/(sigma * sqrt(t)))
    bt <- Vt - at * St
  }
    err_vec[i] <- Vt - max(St - K, 0)
}
sd(err_vec)
