...
set.seed(3)
K <- 1; Tt <- 1; sigma <- 0.2; dt <- 1/52; num_rep <- 10000;
simulations <- c(500, 700, 1000, 2000, 3000, 4000, 5000, 6000)
true_delta_err <- w_05_delta_err_7 <- matrix(c(rep(0, length(simulations)*10)), ncol = length(simulations))
...
for(j in 1:10){
  for(i in 1:length(simulations)){
    set.seed(3)
    at <- 0; bt <- 0; num_sim <- simulations[i]
    St <- c(rep(1, length(num_rep)))
    rand_norm_0 <- rnorm(num_sim)
    S_0 <- K + d * sigma * sqrt(Tt) * rand_norm_0
    S_0 <- S_0 %>% as_tibble() %>% arrange(value) %>% pull()
    
    ### Calculating true delta hedge error
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, S0 = S_0, true_delta, pol_degree = 7)
    ...
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, S0 = S_0, calculate_delta, pol_degree = 3, w = 0.5)
    w_05_delta_err_3[j, i] <- err
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, S0 = S_0, calculate_delta, pol_degree = 4, w = 0.5)
    w_05_delta_err_4[j, i] <- err
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, S0 = S_0, calculate_delta, pol_degree = 5, w = 0.5)
    w_05_delta_err_5[j, i] <- err
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, S0 = S_0, calculate_delta, pol_degree = 6, w = 0.5)
    w_05_delta_err_6[j, i] <- err
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, S0 = S_0, calculate_delta, pol_degree = 7, w = 0.5)
    w_05_delta_err_7[j, i] <- err
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, S0 = S_0, calculate_delta, pol_degree = 8, w = 0.5)
    w_05_delta_err_8[j, i] <- err
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, S0 = S_0, calculate_delta, pol_degree = 9, w = 0.5)
    ...
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, S0 = S_0, calculate_delta_price, pol_degree = 3)
    pricing_delta_err_3[j, i] <- err
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, S0 = S_0, calculate_delta_price, pol_degree = 4)
    pricing_delta_err_4[j, i] <- err
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, S0 = S_0, calculate_delta_price, pol_degree = 5)
    pricing_delta_err_5[j, i] <- err
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, S0 = S_0, calculate_delta_price, pol_degree = 6)
    pricing_delta_err_6[j, i] <- err
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, S0 = S_0, calculate_delta_price, pol_degree = 7)
    pricing_delta_err_7[j, i] <- err
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, S0 = S_0, calculate_delta_price, pol_degree = 8)
    pricing_delta_err_8[j, i] <- err
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, S0 = S_0, calculate_delta_price, pol_degree = 9)
...
plot(y = true_delta_err, simulations, type = 'l', lty = 2, ylim = c(0.01, 0.05), col = "red") +
