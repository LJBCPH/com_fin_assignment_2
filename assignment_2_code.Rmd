...
for(j in 1:10){
  for(i in 1:length(simulations)){
    at <- 0; bt <- 0;num_rep <- 1000; num_sim#simulations[i]
    S_0 <- c(rep(1, length(num_rep)))
    St <- S_0
    
    ### Calculating true delta hedge error
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, true_delta, pol_degree = 7)
    ...
    #   Vt <- at * St + bt
    #   at <- calculate_delta(St, sigma, t, 0.5, pol_degree = 7)$delta
    #   #at <- pnorm((St-K)/(sigma * sqrt(t)))
    #   bt <- Vt - at * St
    # }
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, calculate_delta, pol_degree = 3, w = 0.5)
    w_05_delta_err_3[j, i] <- err
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, calculate_delta, pol_degree = 4, w = 0.5)
    w_05_delta_err_4[j, i] <- err
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, calculate_delta, pol_degree = 5, w = 0.5)
    w_05_delta_err_5[j, i] <- err
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, calculate_delta, pol_degree = 6, w = 0.5)
    w_05_delta_err_6[j, i] <- err
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, calculate_delta, pol_degree = 7, w = 0.5)
    w_05_delta_err_7[j, i] <- err
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, calculate_delta, pol_degree = 8, w = 0.5)
    w_05_delta_err_8[j, i] <- err
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, calculate_delta, pol_degree = 9, w = 0.5)
    ...
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, calculate_delta_price, pol_degree = 3)
    pricing_delta_err_3[j, i] <- err
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, calculate_delta_price, pol_degree = 4)
    pricing_delta_err_4[j, i] <- err
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, calculate_delta_price, pol_degree = 5)
    pricing_delta_err_5[j, i] <- err
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, calculate_delta_price, pol_degree = 6)
    pricing_delta_err_6[j, i] <- err
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, calculate_delta_price, pol_degree = 7)
    pricing_delta_err_7[j, i] <- err
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, calculate_delta_price, pol_degree = 8)
    pricing_delta_err_8[j, i] <- err
    err <- calculate_hedge_error(dt, Tt, num_rep, K, sigma, St, calculate_delta_price, pol_degree = 9)
